...
#include <Arduino.h>
#include "../utils/lvgl/lvgl.h"             // ต้องมีบรรทัดนี้ด้วย ไม่งั้นเพิ่ม lv_area.h อย่างเดียวจะตาย
#include "../utils/lvgl/src/misc/lv_area.h" // <== lv_point_t 
#include "../utils/lvgl/src/misc/lv_area_private.h" // คำสั่ง area ต่างๆ
#include <stdint.h>
#include <cmath>
...
struct Point {
  ...
  bool   is_in(GArea a , int32_t corner_radius=0);
  bool   is_in(int32_t x1, int32_t y1, int32_t x2, int32_t y2, int32_t corner_radius=0);
  bool   is_in(Point p, int32_t w, int32_t h, int32_t corner_radius=0);
  bool   is_in(Point p1, Point p2, int32_t corner_radius=0);  // อยู่บนพื้นที่ระหว่าง p1, p2 หรือไม่
...
class GArea {
  public:
    ...
    GArea& operator=(const area_t& area)      { this->setPoints( area); } 
    bool   operator==(const GArea& area)      { return lv_area_is_equal(&this->_area, &area._area);     }
    bool   operator==(const area_t& area)     { return lv_area_is_equal(&this->_area, &area);           }
    bool   operator!=(const GArea& area)      { return !lv_area_is_equal(&this->_area, &area._area);    }
    bool   operator!=(const area_t& area)     { return !lv_area_is_equal(&this->_area, &area);          }
    ...
    bool intersect(const GArea &a, const GArea&b, GArea* res_area=NULL)  { 
      lv_area_t _res_area;  
      bool res = _lv_area_intersect(&_res_area, &a._area, &b._area );   
      if( res_area !=NULL) {
    ...
    GArea join(const GArea&a, const GArea&b) {
      lv_area_t _res_area;  
      _lv_area_join(&_res_area, &a._area, &b._area );   
    ...
    inline  bool is_on(int32_t x1, int32_t y1, int32_t x2, int32_t y2)  {
    ...
    inline bool is_in(const GArea& a, int32_t radius=0 ) {
      return lv_area_is_in( (const lv_area_t * ) &this->_area, (const lv_area_t * ) &a._area, radius);
    }

    inline  bool is_in(int32_t x1, int32_t y1, int32_t x2, int32_t y2, int32_t radius=0)  {
      lv_area_t a;   lv_area_set(&a, x1, y1, x2, y2);
      return  lv_area_is_in( (const lv_area_t * ) &this->_area, (const lv_area_t * ) &a, radius );
    }
